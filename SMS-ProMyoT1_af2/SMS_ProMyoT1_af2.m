...
% SMS-ProMyoT1, integrating simultaneous multi-slice (SMS) with
...
%This demo will generate SMS-ProMyoT1 for MB 2 and 3 with in-plane
%acceleration (af=2)
...
ProMyoT1Parameters = inputdlg({'FOV_{readout} [mm]','FOV_{phase} [mm]','N_{readout}','N_{phase}' , 'Trigger Delay [ms]'},...
    'ProMyoT1 parameters', [1 20; 1 20; 1 20 ; 1 20; 1 30], {'384','327','256','144', '550'}, opts);

% seq.plot('timeRange', [6.1865 6.2]);
try
    fovr = str2num(ProMyoT1Parameters{1})*1e-3; %[m]
    fovp = str2num(ProMyoT1Parameters{2})*1e-3; %[m]
    Nx =  str2num(ProMyoT1Parameters{3});
    sms    Ny =  str2num(ProMyoT1Parameters{4});
    Tdelay_trig = str2num(ProMyoT1Parameters{5})*1e-3;
catch
        ...
        for zgap = 24 %16:8:60
            ...
            for ipattern = 2%2%size(Nyacquired_matrix,2)
                ...
                if write_flag
                    ...
                    nameseq =  sprintf('SMS_ProMyoT1_MB%1.0f_af%1.1f_NyAcq%.0f_fovr%.0f_fovp%.0f_TD%3.0f_B1%.0f_TR%01.01f_zg%2.0f_st%1.0f_nLacs%2.0fFS%2.0f_TI8i_forkacs_Delay%1.0f_Ns%02.0f_bssfp%01.0fFLASH',  ...
                    ...
                    if delay_ACS
                        path_save = sprintf('SMSProMyoT1/MB%1.0f_B1%01.0f/st%1.0f/zg%2.0f/af%1.1f/Delay_ACS', ...
                            MB,B1max*1e3,thick, bs*thick, afactor);
                    else
                        path_save = sprintf('SMSProMyoT1af2/MB%1.0f_B1%01.0f/st%1.0f/zg%2.0f/af%1.1f/NoDelay_ACS', ...
                            MB,B1max*1e3,thick, bs*thick, afactor);
                    end
